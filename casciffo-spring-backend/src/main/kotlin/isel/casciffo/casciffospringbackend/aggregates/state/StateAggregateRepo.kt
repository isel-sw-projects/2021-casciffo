...
interface StateAggregateRepo: ReactiveCrudRepository<StateAggregate, Int> {
    ...
    @Query(
        "SELECT s.state_id, s.state_name, nps.next_state_id, ns.state_name as next_state_name, r.role_name, r.role_id " +
        "FROM states s " +
        "JOIN next_possible_states nps on s.state_id = nps.origin_state_id " +
        "JOIN states ns on ns.state_id = nps.next_state_id " +
