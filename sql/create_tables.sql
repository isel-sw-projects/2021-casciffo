...
CREATE TABLE user_roles (
    ...
    CONSTRAINT fk_role_name FOREIGN KEY (role_name) REFERENCES role(role_name)
)
...
CREATE TABLE user_notification (
    ...
    title VARCHAR,
    description TEXT,
    CONSTRAINT fk_notified_user FOREIGN KEY(notified_user) REFERENCES user(user_id)
)
...
CREATE TABLE state (
    ...
    CONSTRAINT fk_role FOREIGN KEY(role_responsible_for_advancing) REFERENCES role(role_name)
)
...
CREATE TABLE state_transition (
    ...
    CONSTRAINT fk_state_before FOREIGN KEY (state_before) REFERENCES state(state_name),
    CONSTRAINT fk_state_after FOREIGN KEY (state_after) REFERENCES state(state_name),
)
...
CREATE TABLE investigation_team (
    ...
    investigator_id INT,
    investigator_role VARCHAR, -- team member / principal
    CONSTRAINT fk_investigator_id FOREIGN KEY (investigator_id) REFERENCES user(user_id)
)
...
CREATE TABLE proposal (
    ...
    proposal_type VARCHAR,
    CONSTRAINT fk_state_id FOREIGN KEY(state_id) REFERENCES state(state_name),
    ...
    CONSTRAINT fk_protocol_state FOREIGN KEY(protocol_state) REFERENCES state(state_name),
...
CREATE TABLE comment (
    ...
    CONSTRAINT fk_comment_id FOREIGN KEY(proposal_id) REFERENCES proposal(proposal_id)
)

CREATE TABLE timeline_event (
    event_id SERIAL PRIMARY KEY,
    proposal_id INT,
    event_type VARCHAR, -- deadline / state transition
    event_name VARCHAR,
    deadline_date DATETIME,
    completed_date DATETIME,
    is_over_due BIT,
    days_over_due DATETIME,
    CONSTRAINT  fk_proposal_id FOREIGN KEY(proposal_id) REFERENCES proposal(proposal_id)
)
...
CREATE TABLE proposal_financial_component (
    ...
    promoter_id INT,
    CONSTRAINT fk_proposal_id FOREIGN KEY(proposal_id) REFERENCES proposal(proposal_id),
    CONSTRAINT fk_financial_contract_id FOREIGN KEY (financial_contract_id) REFERENCES file(file_id),
...
CREATE TABLE clinical_research (
    ...
    CONSTRAINT fk_proposal_id FOREIGN KEY(proposal_id) REFERENCES proposal(proposal_id),
    CONSTRAINT fk_state_id FOREIGN KEY(research_state) REFERENCES state(state_name)
)

CREATE TABLE research_state_transitions (
    research_id INT,
    state_transition_id INT,
    motive text, -- reason for canceling / reason for starting such as SIV completed
...
CREATE TABLE team_finance (
    ...
    investigator_id INT, 
    type_of_flow VARCHAR,
    transaction_date DATETIME,
    responsible_for_payment VARCHAR,
    amount FLOAT,
    partition_percentage,
    role_amount FLOAT,
    CONSTRAINT fk_trial_finance_component FOREIGN KEY (trial_financial_component_id) REFERENCES trial_financial_component(financial_id),
    CONSTRAINT fk_investigator_id FOREIGN KEY (investigator_id) REFERENCES user(user_id)
)

CREATE TABLE research_finance (
    research_finance_id SERIAL PRIMARY KEY,
    trial_finance_component_id INT,
    transaction_date DATETIME,
    type_of_flow VARCHAR,
    motive VARCHAR,
    amount FLOAT,
    CONSTRAINT fk_trial_finance_component FOREIGN KEY(trial_finance_component_id) REFERENCES trial_finance_component(financial_id)
)
...
CREATE TABLE addenda (
    ...
    addenda_file_id INT,
    CONSTRAINT fk_research_id FOREIGN KEY(research_id) REFERENCES clinical_research(research_id),
    CONSTRAINT fk_addenda_state FOREIGN KEY(addenda_state) REFERENCES state(state_name),
    CONSTRAINT fk_file_id FOREIGN KEY(addenda_file_id) REFERENCES file(file_id)
)
